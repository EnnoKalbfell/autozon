...
export class LoginMockService {
  ...
  public getUser(isDealer?: boolean): BehaviorSubject<IUser | undefined> {
    let user: IUser;
    isDealer ? user = dealer : user = customer;

    const userSource$ = new BehaviorSubject<IUser | undefined>(user);
    return userSource$;
  }
